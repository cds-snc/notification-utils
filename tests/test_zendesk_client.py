...
@pytest.mark.parametrize(
    "extra_args, expected_tag_list, expected_priority",
    (
        (
            {},
...
def test_create_ticket(
    ...
    assert b64decode(b64_auth.encode()).decode() == "{}/token:{}".format("zd-api-notify@digital.cabinet-office.gov.uk", "testkey")
...
def test_create_ticket_with_user_name_and_email(zendesk_client, rmock, name, zendesk_name):
    ...
    zendesk_client.create_ticket(
        "subject", "message", ticket_type=zendesk_client.TYPE_PROBLEM, user_name=name, user_email="user@example.com"
    )
