...
def test_get_annotated_rows(file_contents, template_type, expected):
    recipients = RecipientCSV(file_contents, template_type=template_type, placeholders=["name"], max_initial_rows_shown=1)
...
def test_get_recipient_respects_order(file_contents, template_type, placeholders, expected_recipients, expected_personalisation):
    recipients = RecipientCSV(file_contents, template_type=template_type, placeholders=placeholders)
...
@pytest.mark.parametrize("placeholders", [None, ["name"]])
@pytest.mark.parametrize(
...
def test_bad_or_missing_data(file_contents, template_type, rows_with_bad_recipients, rows_with_missing_data, partial_instance):
    ...
    sum(
        [
            ...
            for expected, group in [
                ("6502532222", ("phone number", "   PHONENUMBER", "phone_number", "phone-number", "phoneNumber")),
...
def test_ignores_leading_whitespace_in_file(character, name):
    ...
    recipients = RecipientCSV("{}emailaddress\ntest@example.com".format(character), template_type="email")
    first_row = recipients[0]
...
def test_multiple_sms_recipient_columns(international_sms):
    ...
    assert recipients.rows[0].get("phone number").data == ("6502532224")
...
@pytest.mark.parametrize("column_name", ("phone_number", "phonenumber", "phone number", "phone-number", "p h o n e  n u m b e r"))
