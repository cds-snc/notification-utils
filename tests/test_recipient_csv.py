...
def test_multiple_sms_recipient_columns(international_sms):
    ...
    assert recipients.duplicate_recipient_column_headers == {
        'phone number': None, 'phone_number': None
    }
    assert recipients.has_errors
...
def test_multiple_sms_recipient_columns_with_missing_data(column_name):
    ...
    assert recipients.duplicate_recipient_column_headers == list(dict.fromkeys(expected_duplicated_columns))
    assert recipients.has_errors
...
def test_multiple_letter_recipient_columns():
    ...
    assert recipients.has_errors
    assert recipients.duplicate_recipient_column_headers == {
        'address line 1': None, 'Address Line 2': None,
    }
    assert recipients.has_errors
