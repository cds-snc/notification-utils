...
def test_makes_links_out_of_URLs(template_class, url, url_with_entities_replaced):
    assert '<a style="word-wrap: break-word; word-break: break-all;" href="{}">{}</a>'.format(
...
@pytest.mark.parametrize(
    "template_class, extra_args, expected_remove_whitespace_calls",
    [
        (
            PlainTextEmailTemplate,
            {},
...
@pytest.mark.parametrize(
    "template_class, extra_args, expected_calls",
    [
        (
            PlainTextEmailTemplate,
            {},
...
def test_multiple_newlines_in_letters(
    ...
    assert expected_preview_markup in str(LetterPreviewTemplate({"content": content, "subject": "foo"}))
...
def test_letter_preview_uses_non_breaking_hyphens():
    assert "non\u2011breaking" in str(LetterPreviewTemplate({"content": "non-breaking", "subject": "foo"}))
    assert "–" in str(LetterPreviewTemplate({"content": "en dash - not hyphen - when set with spaces", "subject": "foo"}))
    ...
    (
        ...
        (
            ...
            (
                ...
                'Heading <a style="word-wrap: break-word; word-break: break-all;" href="https://example.com">link</a>'
                "</h2>"
            ),
        ),
        (LetterPreviewTemplate, ("<h2>Heading link: <strong>example.com</strong></h2>")),
        (LetterPrintTemplate, ("<h2>Heading link: <strong>example.com</strong></h2>")),
    ),
)
def test_heading_only_template_renders(renderer, expected_content):
    assert expected_content in str(renderer({"subject": "foo", "content": ("# Heading [link](https://example.com)")}))
